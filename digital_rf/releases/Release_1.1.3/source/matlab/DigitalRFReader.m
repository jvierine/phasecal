classdef DigitalRFReader
    ...
    %
    % $Id: DigitalRFReader.m 757 2015-04-03 15:21:08Z brideout $
    ...
    methods
        ...
        function samples_per_file = get_samples_per_file(obj, channel)
            % get_samples_per_file returns samples_per_file for given
            % channel
            drf_chan = obj.channel_map(channel);
        ...
        function sample_rate = get_sample_rate(obj, channel)
            % get_sample_rate returns sample_rate in Hz for given
            % channel
            drf_chan = obj.channel_map(channel);
        ...
        function is_complex = get_is_complex(obj, channel)
            % get_is_complex returns is_complex (1 or 0) for given
            % channel
            drf_chan = obj.channel_map(channel);
        ...
        function num_subchannels = get_num_subchannels(obj, channel)
            % get_num_subchannels returns num_subchannels (1 or greater) for given
            % channel
            drf_chan = obj.channel_map(channel);
        ...
        function gap_array = get_gap_array(obj, channel, first_sample, last_sample)
            % get_gap_array returns a N x 2 array of int64, where each row
            % represents a data gap, with column 1 being the first missing
        ...
        function continuous_array = get_continuous_array(obj, channel, first_sample, last_sample)
            % get_continuous_array returns a N x 2 array of int64, where each row
            % represents a block of continuous daya, with column 1 being
