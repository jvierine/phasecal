...
$Id: test_millstone_data_access.py 525 2014-08-04 18:22:16Z brideout $
"""
...
print('Test getting small subsets of read_vector_raw at different places')
for i in range(5):

    start_index = b[0] + long(0.03*i*(b[1]-b[0]))
    end_index = start_index + 10000000
    t = time.time()
    vector = o.read_vector_raw(start_index,end_index-start_index,test_chan)
    print('read_vector_raw between %i and %i are len=%i, value=<%s>' % (start_index, end_index, len(vector), str(vector)))
    print('took %f' % (time.time() - t))

print('Test getting small subsets of contiguous read_vector_raw (so assuming no reload needed after the first')
