...
$Id: digital_rf_hdf5.py 524 2014-08-04 18:15:20Z brideout $
"""
...
class read_hdf5:
    ...
    def reload(self, start_sample=None, sample_length=None, channel=None):
        ...
        updated detailed metadata as needed.  However, if self._load_all_metadata is True, then update all metadata.
        ...
        if high_level:
            self._high_level_reload(complete_update=self._load_all_metadata)
        else:
    ...
    def read_vector_raw(self, unix_sample, vector_length, channel_name):
        ...
        """
        try:
            return(self._read_vector(unix_sample, vector_length, channel_name, require_all_data=True))
        except _MissingMetadata:
            self.reload(unix_sample, vector_length, channel_name)
            try:
                return(self._read_vector(unix_sample, vector_length, channel_name))
            except _MissingMetadata:
                raise IOError, str(sys.exc_info()[1])
    ...
    def read_vector_c81d(self, unix_sample, vector_length, channel_name, subchannel=0):
        ...
        This method now deprecated - issues a UserWarning if called.
        ...
        This is possible because metadata on which this call is based might be out of date.
        """
        warnings.warn('The method read_vector_c81d is deprecated.  Instead use read_vector() to get all data as complex8, or read_vector_raw to get data in format stored in Hdf5.',
                      UserWarning)
        z = self.read_vector_raw(unix_sample, vector_length, channel_name)
    ...
    def _read_vector(self, unix_sample, vector_length, channel_name, require_all_data=False):
        ...
        Inputs:
            ...
            require_all_data - if True, raise _MissingMetadata exception if asking for data beyond the end
                of the metadata.  If False (the default), only return based on existing metadata,
        ...
        if require_all_data:
            ...
            if last_top_level_metadata.unix_start_sample + last_top_level_metadata.sample_extent < unix_sample + vector_length:
                raise _MissingMetadata, 'request in _read_vector beyond existing Metadata'
        ...
        for top_level_dir in channel_metadata.top_level_dir_meta_list:
            ...
            if unix_sample + vector_length < top_level_dir.unix_start_sample:
                ...
                continue
            try:
                # run the faster version if require_all_metadata == True, version with more metadata checking if False
                this_array, this_unix_sample = top_level_dir.get_continuous_vector(max(unix_sample, top_level_dir.unix_start_sample),
                                                                                   ...
                                                                                   not require_all_data)
            except:
                raise _MissingMetadata, str(sys.exc_info()[1])
    ...
    def _high_level_reload(self, complete_update=False):
        """_high_level_reload updates the attributes self._channel_location_dict and self._channel_dict using only
        high level metadata if complete_update = False (the default). If complete update is True, update all
        possible metadata.
        """
        ...
        for channel_name in channel_dict.keys():
            if not self._channel_dict.has_key(channel_name):
                ...
                new_channel_metadata.update(complete_update=complete_update)
                ...
                chan_obj.update(complete_update=complete_update)
...
class _channel_metadata:
    ...
    def update(self, start_sample=None, sample_length=None, complete_update=False):
        ...
        for top_level_meta in self.top_level_dir_meta_list:
            top_level_meta.update(start_sample, sample_length, complete_update)
...
class _top_level_dir_metadata:
    ...
    def get_continuous_vector(self, start_unix_sample, stop_unix_sample, update_metadata=False):
        ...
        Inputs:
            ...
            update_metadata - if True, update metadata in this top_level_dir.  If False (the default), do not
