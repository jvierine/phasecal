"""test_digital_rf_realtime.py is a script to test the digital_rf_hdf5 module
under realtime conditions

In this realtime test, RF files are added to last subdirectory, but no new subdirectories added.

$Id: test_digital_rf_realtime.py 525 2014-08-04 18:22:16Z brideout $
"""

usage = 'python test_digital_rf_realtime.py <0 for fast metadata, 1 for full metadata>'
...
print('now try to read extra data')
try:
    result = testReadObj.read_vector_raw(cont_data_arr[0][0], cont_data_arr[0][1]+100, 'junk1')
    print('got %i samples' % (len(result)))
except:
    traceback.print_exc()
