 ...
 *
 * $Id: test_rf_write_hdf5.c 666 2014-10-15 17:48:27Z brideout $
 */
...
int main (int argc, char *argv[])
{
	...
	sample_rate = 1.0E9;
	global_index = (uint64_t)(1394368230 * sample_rate) + 1; /* should represent 2014-03-09 12:30:30  and 1E3 picoseconds*/
	...
	printf("%i-%i-%i %i:%i:%i pico: %" PRIu64 "\n", year, month, day, hour, minute, second, picosecond);

	/* change sample_rate to 100HZ so we can work with small files */
	sample_rate = 100.0;
	global_index = (uint64_t)(1394368230 * sample_rate) + 1; /* should represent 2014-03-09 12:30:30  and 1E10 picoseconds*/


	printf("Test 0 - simple single write to multiple files, no compress, no checksum, files_per_directory=0, - channel 0\n");
	system("rm -rf /tmp/hdf5 ; mkdir /tmp/hdf5 ; mkdir /tmp/hdf5/junk0");
	data_object = digital_rf_create_write_hdf5("/tmp/hdf5/junk0", H5T_NATIVE_INT, 40, 0, global_index, sample_rate, "FAKE_UUID_0", 0, 0, 1, 1, 0);
	if (!data_object)
		exit(-1);
	for (i=0; i<7; i++)
	{
	...
	data_object = digital_rf_create_write_hdf5("/tmp/hdf5/junk0.1", H5T_NATIVE_INT, 40, 10, global_index, sample_rate, "FAKE_UUID_0.1", 0, 0, 1, ARR_SIZE, 0);
	if (!data_object)
		exit(-1);
	for (i=0; i<7; i++)
	{
	...
	data_object = digital_rf_create_write_hdf5("/tmp/hdf5/junk1", H5T_NATIVE_CHAR, 40, 10, global_index, sample_rate, "FAKE_UUID_1", 0, 0, 1, 1, 1);
	if (!data_object)
		exit(-1);
	result = digital_rf_write_hdf5(data_object, vector_leading_edge_index, data_char, vector_length);
	if (result)
		exit(-1);
	result = digital_rf_write_hdf5(data_object, vector_leading_edge_index + 120, data_char, vector_length);
	if (result)
		exit(-1);
	digital_rf_close_write_hdf5(data_object);
	printf("done test 1\n");
	...
	data_object = digital_rf_create_write_hdf5("/tmp/hdf5/junk1.0", H5T_NATIVE_CHAR, 40, 10, global_index, sample_rate, "FAKE_UUID_1.0", 0, 0, 1, ARR_SIZE, 1);
	if (!data_object)
		exit(-1);
	...
	data_object = digital_rf_create_write_hdf5("/tmp/hdf5/junk1.1", H5T_NATIVE_CHAR, 40, 10, global_index, sample_rate, "FAKE_UUID_1.1", 0, 0, 0, 1, 1);
	if (!data_object)
		exit(-1);
	for (i=0; i<10; i++)
	{
	...
	data_object = digital_rf_create_write_hdf5("/tmp/hdf5/junk1.1.0", H5T_NATIVE_CHAR, 40, 10, global_index, sample_rate, "FAKE_UUID_1.1", 0, 0, 0, ARR_SIZE, 1);
	if (!data_object)
		exit(-1);
	for (i=0; i<10; i++)
	{
	...
	data_object = digital_rf_create_write_hdf5("/tmp/hdf5/junk2", H5T_NATIVE_SHORT, 40, 10, global_index, sample_rate, "FAKE_UUID_2", 1, 0, 1, 1, 1);
	if (!data_object)
		exit(-1);
	...
	data_object = digital_rf_create_write_hdf5("/tmp/hdf5/junk3", H5T_NATIVE_INT, 40, 10, global_index, sample_rate, "FAKE_UUID_3", 0, 1, 1, 1, 1);
	if (!data_object)
		exit(-1);
	...
	data_object = digital_rf_create_write_hdf5("/tmp/hdf5/junk4", H5T_NATIVE_LLONG, 40, 10, global_index, sample_rate, "FAKE_UUID_4.1", 6, 1, 1, 1, 1);
	if (!data_object)
		exit(-1);
	...
	data_object = digital_rf_create_write_hdf5("/tmp/hdf5/junk4.1", H5T_NATIVE_LLONG, 40, 10, global_index, sample_rate, "FAKE_UUID_4.1", 6, 1, 0, 1, 1);
	if (!data_object)
		exit(-1);
	...
	data_object = digital_rf_create_write_hdf5("/tmp/hdf5/junk4.2", H5T_NATIVE_LLONG, 40, 10, global_index, sample_rate, "FAKE_UUID_4.2", 6, 1, 0, 1, 1);
	if (!data_object)
		exit(-1);
	init_block_indices(global_index_arr, block_index_arr, 0);
	...
	data_object = digital_rf_create_write_hdf5("/tmp/hdf5/junk5", H5T_NATIVE_UINT, 40, 10, global_index, sample_rate, "FAKE_UUID_5", 0, 0, 1, 1, 1);
	if (!data_object)
		exit(-1);
	...
	data_object = digital_rf_create_write_hdf5("/tmp/hdf5/junk5.1", H5T_NATIVE_UINT, 40, 10, global_index, sample_rate, "FAKE_UUID_5.1", 0, 0, 1, 1, 1);
	if (!data_object)
		exit(-1);
	result = digital_rf_write_hdf5(data_object, vector_leading_edge_index, data_uint, vector_length);
	if (result)
	...
	data_object = digital_rf_create_write_hdf5("/tmp/hdf5/junk5.2", H5T_NATIVE_UINT, 40, 10, global_index, sample_rate, "FAKE_UUID_5.2", 0, 0, 1, 1, 1);
	if (!data_object)
		exit(-1);
	for (i=0; i<5; i++)
	{
	...
	data_object = digital_rf_create_write_hdf5("/tmp/hdf5/junk6", H5T_NATIVE_DOUBLE, 40, 10, global_index, sample_rate, "FAKE_UUID_6", 9, 1, 1, 1, 1);
	if (!data_object)
		exit(-1);
	...
	data_object = digital_rf_create_write_hdf5("/tmp/hdf5/junk7", H5T_NATIVE_DOUBLE, 40, 10, global_index, sample_rate, "FAKE_UUID_7", 9, 1, 1, 1, 1);
	if (!data_object)
		exit(-1);
	result = digital_rf_write_hdf5(data_object, vector_leading_edge_index, data_double, vector_length);
	if (result)
	...
	data_object = digital_rf_create_write_hdf5("/tmp/hdf5/junk8", H5T_NATIVE_DOUBLE, 40, 10, global_index, sample_rate, "FAKE_UUID_8", 9, 1, 1, 1, 1);
	if (!data_object)
		exit(-1);
	result = digital_rf_write_hdf5(data_object, vector_leading_edge_index, data_double, vector_length);
	if (result)
	...
	data_object = digital_rf_create_write_hdf5("/nosuchdirectory/junk", H5T_NATIVE_DOUBLE, 40, 10, global_index, sample_rate, "FAKE_UUID_9", 9, 1, 1, 1, 1);
	if (!data_object)
	...
	data_object = digital_rf_create_write_hdf5("/tmp/hdf5/junk0/rf_data_seq_000000.hdf5", H5T_NATIVE_DOUBLE, 40, 10, global_index, sample_rate, "FAKE_UUID_10", 9, 1, 1, 1, 1);
	if (!data_object)
	...
	data_object = digital_rf_create_write_hdf5("/tmp/hdf5/junk6", H5T_NATIVE_DOUBLE, 40, 10, global_index, sample_rate, "FAKE_UUID_10", 10, 1, 1, 1, 1);
	if (!data_object)
	...
	data_object = digital_rf_create_write_hdf5("/tmp/hdf5/junk13", H5T_NATIVE_LLONG, 40, 10, global_index, sample_rate, "FAKE_UUID_4", 6, 1, 0, 1, 1);
	if (!data_object)
		exit(-1);
	init_block_indices(global_index_arr, block_index_arr, 0);
	result = digital_rf_write_blocks_hdf5(data_object, global_index_arr, block_index_arr, 10, single_int64, vector_length);
	...
	printf("Test 14 - try to write using digital_rf_write_blocks_hdf5, but with block data indices that advance slower than the global index - channel 13\n");
		...
		data_object = digital_rf_create_write_hdf5("/tmp/hdf5/junk14", H5T_NATIVE_LLONG, 40, 10, global_index, sample_rate, "FAKE_UUID_4", 6, 1, 0, 1, 1);
		if (!data_object)
			exit(-1);
